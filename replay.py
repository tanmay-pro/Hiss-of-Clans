...
if __name__ == "__main__":
    ...
    mainMap = map(GAME_SCREEN_WIDTH, GAME_SCREEN_HEIGHT,
                  GAME_HORIZONTAL_BOUNDARY, GAME_VERTCIAL_BOUNDARY)
    mainMap.createMap()

    mainKing = king(KING_STARTING_X, KING_STARTING_Y,
                    KING_HEALTH, KING_SPEED, KING_ATTACK)
    ...
    for i in range(0, NUMBER_WALLS_Y):
        arrayWalls.append(
            wall(WALL_STARTING1X, WALL_STARTING1Y + i, WALL_HEALTH))
    for i in range(0, NUMBER_WALLS_Y):
        arrayWalls.append(
            wall(WALL_STARTING2X, WALL_STARTING1Y + i, WALL_HEALTH))
    for i in range(0, NUMBER_WALLS_X):
        arrayWalls.append(
            wall(WALL_STARTING1X + i, WALL_STARTING1Y, WALL_HEALTH))
    for i in range(0, NUMBER_WALLS_X):
        arrayWalls.append(
            wall(WALL_STARTING1X + i, WALL_STARTING2Y, WALL_HEALTH))
    ...
    cannon1 = cannon(CANNON_1X, CANNON_1Y, CANNON_HEALTH,
                     CANNON_DAMAGE, CANNON_RANGE)
    cannon2 = cannon(CANNON_2X, CANNON_2Y, CANNON_HEALTH,
                     CANNON_DAMAGE, CANNON_RANGE)
    ...
    while gameStatus == "playing":
        ...
        print("Number of barbarians you can still spawn: " +
              str(MAX_BARBARIANS - currBarbarians))
        print("Number of spells you can still use: " +
              str(MAX_SPELLS - currSpellsUsed))
        for everyBarbarian in arrayBarbarians:
            if not everyBarbarian.isDead:
                everyBarbarian.move(mainMap, mainTownHall,
                                    arrayHuts, arrayWalls, arrayCannons)
                everyBarbarian.attack(
                    mainMap, mainTownHall, arrayHuts, arrayWalls, arrayCannons)
        ...
        if ch == "w" or ch == "a" or ch == "s" or ch == "d":
            ...
            if not mainKing.isDead:
                mainKing.attack(mainMap, mainTownHall,
                                arrayHuts, arrayWalls, arrayCannons)
        elif (ch == "1" or ch == "2" or ch == "3") and currBarbarians < MAX_BARBARIANS:
            currBarbarians += 1
            if ch == "1":
                barbarian1 = barbarian(
                    sp1.positionX, sp1.positionY, BARBARIAN_HEALTH, BARBARIAN_SPEED, BARBARIAN_ATTACK)
            elif ch == "2":
                barbarian1 = barbarian(
                    sp2.positionX, sp2.positionY, BARBARIAN_HEALTH, BARBARIAN_SPEED, BARBARIAN_ATTACK)
            elif ch == "3":
                barbarian1 = barbarian(
                    sp3.positionX, sp3.positionY, BARBARIAN_HEALTH, BARBARIAN_SPEED, BARBARIAN_ATTACK)
            ...
            if not mainKing.isDead:
                mainKing.attackMajor(mainMap, mainTownHall,
                                     arrayHuts, arrayWalls, arrayCannons)
